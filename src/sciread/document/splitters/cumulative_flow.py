...
class CumulativeFlowSplitter(BaseSplitter):
    """
    ...
    def __init__(
        ...
        """
        Initialize CumulativeFlow splitter.

        Args:
            ollama_client: OllamaClient instance for embeddings (optional)
            similarity_threshold: Threshold for cumulative similarity (segment vs next sentence)
    ...
    def _create_chunk_from_sentences(
        ...
        chunk = Chunk(
            content=content,
            chunk_name=f"cumulative_chunk_{len(content)}",  # Give it a searchable name
            position=0,  # Will be set when added to chunks list
            char_range=(start_char, end_char),
            confidence=confidence,
            metadata={**metadata, "splitter": "cumulative_flow"},
        )
        return chunk
    ...
    def _create_single_chunk(self, text: str) -> list[Chunk]:
        ...
        chunk = Chunk(
            ...
            confidence=0.8,
            metadata={"splitter": "cumulative_flow"},
        )
        return [chunk]
