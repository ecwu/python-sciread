...
class AgentInsight(BaseModel):
    ...
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence level from 0 to 1"
    )
    supporting_evidence: List[str] = Field(
        default_factory=list, description="Evidence supporting the insight"
    )
    related_sections: List[str] = Field(
        default_factory=list, description="Related document sections"
    )
    questions_raised: List[str] = Field(
        default_factory=list, description="Questions this insight raises"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the insight was generated"
    )
...
class Question(BaseModel):
    ...
    question_type: str = Field(
        ..., description="Type of question (clarification, challenge, extension)"
    )
    priority: float = Field(
        ..., ge=0.0, le=1.0, description="Priority score from 0 to 1"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the question was asked"
    )
    requires_response: bool = Field(
        default=True, description="Whether this question requires a response"
    )
...
class Response(BaseModel):
    ...
    from_agent: AgentPersonality = Field(
        ..., description="Agent providing the response"
    )
    content: str = Field(..., description="The response content")
    stance: str = Field(
        ..., description="Response stance (agree, disagree, clarify, modify)"
    )
    revised_insight: Optional[str] = Field(
        None, description="Revised insight if applicable"
    )
    confidence: float = Field(
        ..., ge=0.0, le=1.0, description="Confidence in the response"
    )
    timestamp: datetime = Field(
        default_factory=datetime.now, description="When the response was given"
    )
...
class DiscussionState(BaseModel):
    ...
    current_phase: DiscussionPhase = Field(
        ..., description="Current phase of discussion"
    )
    ...
    insights: List[AgentInsight] = Field(
        default_factory=list, description="All insights generated"
    )
    questions: List[Question] = Field(
        default_factory=list, description="All questions asked"
    )
    responses: List[Response] = Field(
        default_factory=list, description="All responses given"
    )
    ...
    convergence_score: float = Field(
        default=0.0, ge=0.0, le=1.0, description="How converged the discussion is"
    )
    start_time: datetime = Field(
        default_factory=datetime.now, description="Discussion start time"
    )
...
class ConsensusPoint(BaseModel):
    ...
    supporting_agents: List[AgentPersonality] = Field(
        ..., description="Agents supporting this consensus"
    )
    opposing_agents: List[AgentPersonality] = Field(
        default_factory=list, description="Agents opposing this consensus"
    )
    strength: float = Field(..., ge=0.0, le=1.0, description="Strength of consensus")
    evidence: List[str] = Field(
        default_factory=list, description="Evidence supporting the consensus"
    )
...
class DivergentView(BaseModel):
    ...
    counter_arguments: List[str] = Field(
        default_factory=list, description="Counter arguments from other agents"
    )
...
class DiscussionResult(BaseModel):
    ...
    key_contributions: List[str] = Field(
        default_factory=list, description="Key contributions identified"
    )
    ...
    divergent_views: List[DivergentView] = Field(
        default_factory=list, description="Unresolved divergent views"
    )
    final_insights: List[AgentInsight] = Field(
        default_factory=list, description="Most important insights"
    )
    confidence_score: float = Field(
        ..., ge=0.0, le=1.0, description="Overall confidence in results"
    )
    discussion_metadata: Dict[str, Any] = Field(
        default_factory=dict, description="Discussion metadata"
    )
    completion_time: datetime = Field(
        default_factory=datetime.now, description="When discussion completed"
    )
