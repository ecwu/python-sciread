...
class RAGReActAgentOutput(BaseModel):
    ...
    report_section: str = Field(description="New content generated based on the retrieved content")
    next_search_query: str = Field(description="Search query for the next iteration (empty if should_stop is True)")
    reasoning: str = Field(description="Explanation of why the agent made these choices (stop decision and query selection)")
