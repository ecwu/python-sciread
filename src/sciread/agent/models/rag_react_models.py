...
class RAGReActAgentInput(BaseModel):
    ...
    retrieved_content: str = Field(
        description="Content retrieved from semantic search for this iteration"
    )
    search_query: str = Field(
        description="The search query used to retrieve the content"
    )
    search_results_summary: str = Field(
        description="Summary of what search results were found"
    )
    ...
    previous_queries: list[str] = Field(
        description="List of search queries already used in previous iterations"
    )
...
class RAGReActAgentOutput(BaseModel):
    ...
    next_search_query: str = Field(
        description="Search query for the next iteration (empty if should_stop is True)"
    )
    reasoning: str = Field(
        description="Explanation of why the agent made these choices (stop decision, content decision, query selection)"
    )
    search_strategy: str = Field(
        description="Brief description of what information the next search is targeting"
    )
