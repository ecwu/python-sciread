...
def run_rag_react_analysis(
    ...
    """Run RAG ReAct analysis on a document file.

    This function provides a synchronous interface to the RAG ReAct agent,
    which uses semantic search and retrieval-augmented generation to analyze
    academic papers intelligently.

    Args:
        document_file: Path to the document file to analyze (PDF or TXT)
        task: Analysis task or question about the document
        model: Model identifier for the LLM provider (default: "deepseek-chat")
        max_loops: Maximum number of analysis iterations (default: 8)
        show_progress: Whether to show progress during analysis (default: True)
    ...
    logger.debug(f"Task: {task_display}")
    logger.debug(f"Configuration: model={model}, max_loops={max_loops}, show_progress={show_progress}")

    try:
        result = analyze_document_with_rag_react(
            document_file,
            task,
            model=model,
            max_loops=max_loops,
            to_markdown=False,  # Always False for RAG to avoid Mineru API
            show_progress=show_progress,
        )
