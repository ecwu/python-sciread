...
from sciread.providers.llm.deepseek import DeepSeekProvider


class TestDeepSeekProvider:
    """Test cases for DeepSeek provider."""

    @patch("sciread.providers.llm.deepseek.get_config")
    def test_create_model_success(self, mock_config):
        ...
        with (
            patch("sciread.providers.llm.deepseek.OpenAIChatModel") as mock_model_class,
            patch("sciread.providers.llm.deepseek.PydanticDeepSeekProvider") as mock_provider,
        ):
            ...
            result = DeepSeekProvider.create_model("deepseek-v4-flash")
    ...
    def test_create_model_with_custom_base_url(self, mock_config):
        ...
        with (
            ...
            DeepSeekProvider.create_model("deepseek-v4-flash", temperature=0.7)
    ...
    @patch("sciread.providers.llm.deepseek.get_config")
    def test_create_model_preserves_explicit_thinking_setting(self, mock_config):
        """Explicit thinking settings should override the DeepSeek v4 default."""
        mock_config.return_value.get_provider_config.return_value.base_url = "https://api.deepseek.com"
        mock_config.return_value.get_api_key.return_value = "test-api-key"
        ...
        with (
            ...
            DeepSeekProvider.create_model("deepseek-v4-flash", settings={"thinking": True})
    ...
    def test_create_model_missing_api_key(self, mock_config):
        ...
        with pytest.raises(ValueError, match="API key not found"):
            DeepSeekProvider.create_model("deepseek-v4-flash")
    ...
    def test_get_supported_models(self):
        ...
        assert models["deepseek-v4-flash"] == "DeepSeek Chat Model"

    def test_is_model_supported(self):
        """Test checking if model is supported."""
        assert DeepSeekProvider.is_model_supported("deepseek-v4-flash")
        assert DeepSeekProvider.is_model_supported("deepseek-v4-pro")
